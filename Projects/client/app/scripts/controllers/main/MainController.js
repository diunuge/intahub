(function (module) {
    cloudPOS.controllers = _.extend(module, {
        MainController: function (scope, location, sessionManager, translate, $rootScope, localStorageService, keyboardManager, $idle, tmhDynamicLocale, 
            ...
            scope.langs = cloudPOS.models.Langs;
            if (localStorageService.getFromLocalStorage('Language')) {
                var temp = localStorageService.getFromLocalStorage('Language');
                for (var i in cloudPOS.models.Langs) {
                    if (cloudPOS.models.Langs[i].code == temp.code) {
                        scope.optlang = cloudPOS.models.Langs[i];
                        tmhDynamicLocale.set(cloudPOS.models.Langs[i].code);
                        }
                }
            } else {
