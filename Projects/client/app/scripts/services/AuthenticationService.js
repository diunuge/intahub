(function (module) {
    cloudPOS.services = _.extend(module, {
        AuthenticationService: function (scope, httpService, SECURITY, localStorageService,timeout, webStorage) {
            ...
            var getAccessToken = function(){
                ...
                httpService.post( "/api/oauth/token?&client_id=community-app&grant_type=refresh_token&client_secret=123&refresh_token=" + refreshToken)
            ...
            this.authenticateWithUsernamePassword = function (credentials) {
        		...
        		if(SECURITY === 'oauth'){
	                httpService.post( "/api/oauth/token?username=" + credentials.username + "&password=" + credentials.password +"&client_id=community-app&grant_type=password&client_secret=123")
    ...
    cloudPOS.ng.services.service('AuthenticationService', ['$rootScope', 'HttpService', 'SECURITY', 'localStorageService','$timeout','webStorage', cloudPOS.services.AuthenticationService]).run(function ($log) {
